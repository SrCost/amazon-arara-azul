// fnrh-checkout — PATCH /hospedes/{hospede_id_fnrh}/checkout
...
serve(async (req) => {
  ...
  try {
    ...
    const data = await fnrhFetch({
      method: "PATCH",
      path: `/hospedes/${reservation.hospede_id_fnrh}/checkout`,
      body: { dataHoraCheckout: dataHora },
    ...
    await admin
      .from("reservations")
      .update({
        situacao_fnrh: "CHECKOUT_REALIZADO",
        fnrh_checkout_em: dataHora,
